...
from django.template.base import TemplateSyntaxError, Node, Variable, token_kwargs
...
ATTRS_RE = re.compile(
    r'(?P<attr>[-\w]+)(\s*=\s*[\'"](?P<val>.*?)[\'"])?', re.MULTILINE | re.DOTALL
)
    ...
    def render(self, context):  # noqa D102
        ...
        with context.push(
            ...
            for included_list in children:
                ...
                children = (
                    node for node in included.nodelist if isinstance(node, FormPartNode)
                )
                _render_parts(context, children)
    ...
    def __init__(self, parser, token):  # noqa D102
        ...
        if len(bits) > 5:
            raise TemplateSyntaxError(
                ...
                " default attrs_dict ), got: {}".format(bits[0], ",".join(bits[1:]))
            )
    ...
    def __init__(self, parser, token):  # noqa D102
        ...
        if len(bits) > 5:
            raise TemplateSyntaxError(
                ...
                " 'attr_name' action ), got: {}".format(bits[0], ",".join(bits[1:]))
            )
