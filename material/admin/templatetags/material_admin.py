...
def get_admin_site():
    """TODO: Remove."""
    site_module = getattr(settings, "MATERIAL_ADMIN_SITE", "django.contrib.admin.site")
...
def date_hierarchy(cl):
    ...
    if cl.date_hierarchy:
        ...
        dates_or_datetimes = (
            "datetimes" if isinstance(field, models.DateTimeField) else "dates"
        )
...
def admin_related_field_urls(bound_field):
    ...
    if rel_widget.can_change_related:
        change_related_template_url = rel_widget.get_related_url(
            info, "change", "__fk__"
        )
        context.update(
    ...
    if rel_widget.can_delete_related:
        delete_related_template_url = rel_widget.get_related_url(
            info, "delete", "__fk__"
        )
        context.update(
